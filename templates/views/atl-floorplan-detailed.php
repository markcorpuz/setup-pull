...
// class
$cs = array(
	'manual_class'		=> 'floorplan-detailed',
...
echo '<div'.$classes.$inline_style.'>';
	...
	if( !empty( $atl_cf_plan_group ) ) {
		echo '<div class="item-group">'.$atl_cf_plan_group.'</div>';
	}
	...
	// CTA
	echo '<div class="item-cta"><a class="item-cta-link" href="'.get_the_permalink( $pid ).'">MORE DETAILS</a></div>';
	...
	if( !empty( $plan_pic ) ) {
		...
		$ppic = wp_get_attachment_image_src( $plan_pic, !empty( $initial_size ) ? $initial_size : 'medium' );
